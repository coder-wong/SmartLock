...
/// sdk configuration editor生成的宏定义
#include "sdkconfig.h"
                                          ...
                                          esp_ble_gatts_cb_param_t *param)
{
  ...
  // 当手机蓝牙调试助手向esp32发送数据时，会触发下面的分支执行
  case ESP_GATTS_WRITE_EVT:
  {
    ...
    if (!param->write.is_prep)
    {
      ...
      /// param->write.value中包含了手机发送给esp32的数据
      // 蓝牙开锁的逻辑
      ...
      // 通过蓝牙控制开始录入指纹
      if (!memcmp(param->write.value, "addfp", 5))
      {
        is_enroll_flag = 1;
      }
      // 蓝牙修改密码的逻辑
      // chgpwd ==> change password
      if (!memcmp(param->write.value, "chgpwd+", 7))
      {
        /// 初始化nvs flash句柄
        ...
        for (int i = 0; i < 6; i++)
        {
          pswd[i] = param->write.value[i + 7]; // 因为前面有"chgpwd+"所以+7
        }
        pswd[6] = '\0';
        nvs_set_str(password_nvs_handle, "password", pswd);
                                ...
                                esp_ble_gatts_cb_param_t *param)
{
  ...
  // 处理非注册应用事件，
  // 处理的是客户端的连接事件，断开连接事件，读事件，写事件，等等。
  do
  {
    int idx;
    // PROFILE_NUM表示应用的数量
    for (idx = 0; idx < PROFILE_NUM; idx++)
    {
          ...
          gatts_if == gl_profile_tab[idx].gatts_if)
      {
        ...
        {
          // 如果应用的gatts_cb函数指针字段非空，则调用此函数，来处理这个应用的各种事件
